...
def student_new_record(): 
    ...
    Label(screen4, text="", bg='#174873', width='85', height='25').place(x=0, y=127) 
        ...
        if university.get() == "--select your university--": # checking for selection of university 
            ...
            if tnc.get(): # checking for acceptance of agreement 
                if re.match("^.+@(\[?)[a-zA-Z0-9-.]+.([a-zA-Z]{2,3}|[0-9]{1,3})(]?)$", email.get()):# validating the email
                    connection = pymysql.connect(host="localhost", user="root", passwd="", database="edumate") # database connection
                    cursor = connection.cursor() 
                    select_query = "SELECT * FROM student_details where email = '" + email.get() + "';" # queries for retrieving values
                    cursor.execute(select_query) # executing the queries
                    student_info = cursor.fetchall()
                    connection.commit() # commiting the connection then closing it.
                    connection.close()
                    if student_info:
                        #messagebox.showerror("Error", "Enter email already exist")
                        Label(screen1, text="Enter email already exist", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                        return
...
def welcome_page(student_info): 
    ...
    Label(screen2, text="", bg='#174873', width='28', height='35').place(x=0, y=96) 
    ...
    photo= Image.open("C:\\Users\\pints\\Desktop\\python pro\\classes\\project_gui\\bg.png")
    photo= photo.resize((180,300), Image.ANTIALIAS)
    photo1 = ImageTk.PhotoImage(photo) # opening right side image - Note: If image is in same folder then no need to mention the full path 
    ...
    img = Image.open("C:\\Users\\pints\\Desktop\\python pro\\classes\\project_gui\\welcome_page.png")
    img = img.resize((400,522), Image.ANTIALIAS)
    photoImg =  ImageTk.PhotoImage(img)
    #photo = PhotoImage(file="C:\\Users\\pints\\Desktop\\welcome_page.png") # opening left side image - Note: If image is in same folder then no need to mention the full path
    label = Label(screen2, image=photoImg, text="") # attaching image to the label 
    label.place(x=200, y=78) 
    label.image = photoImg # it is necessary in Tkinter to keep a instance of image to display image in label 
    ...
    Button(screen2, text='Check your result', width=20, font=("Open Sans", 13, 'bold'), bg='brown', fg='white',command=student_records).place(x=270, y=300)     
# registration window
def register():
    ...
    connection = pymysql.connect(host="localhost", user="root", passwd="", database="edumate") # database connection
    cursor = connection.cursor() 
    select_query = "SELECT university FROM student_details;"
    cursor.execute(select_query) # executing the queries
    columns = cursor.fetchall()
    for column in columns:
       list1.append(list(column))
    #list1=set(list1)
    '''
    ###         
...
def main_screen():
    ...
    Label(screen, text="", bg='#174873',width='65', height='19').place(x=60,y=95)#blue bg in middle of window
